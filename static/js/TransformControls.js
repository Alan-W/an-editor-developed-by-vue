...
( function () {
	...
	THREE.TransformControls = function ( camera, domElement ) {
		...
		this.update = function () {
			...
			//console.log('奇怪的地方',worldPosition);
			if(!scope.object.modeled){//让圆柱 正方体 球 的轴心 在 底部
				this.position.y = 0;
			}
			/*if(!scope.object.modeled&&scope.object.geometry.type == "CylinderBufferGeometry"){
				scope.object.scale.x = scope.object.scale.x;
			}*/
		...
		function onPointerHover( event ) {
			//console.log("源码源码源码源码源码源码onPointerHover");

			if ( scope.object === undefined || scope._dragging === true || ( event.button !== undefined && event.button !== 0 ) ) return;
		...
		function onPointerDown( event ) {
			//console.log("源码源码源码源码源码源码onPointerDown");
			if ( scope.object === undefined || scope._dragging === true || ( event.button !== undefined && event.button !== 0 ) ) return;
			...
			if ( pointer.button === 0 || pointer.button === undefined ) {
				...
				if ( intersect ) {
					...
					if ( planeIntersect ) {
						...
						console.log(' the scope.object.parent is : ---- ', scope.object.parent);
		...
		function onPointerMove( event ) {
			//console.log("源码源码源码源码源码源码onPointerMove",scope._dragging);
			if ( scope.object === undefined || scope.axis === null || scope._dragging === false || ( event.button !== undefined && event.button !== 0 ) ) return;
			...
			if ( _mode === "translate" ) {
				...
				window.curMeshPosition = {
					x: scope.object.position.x,
				...
				if ( scope.axis === "E" ) {
					...
					tempQuaternion.setFromRotationMatrix( tempMatrix.getInverse( parentRotationMatrix ) );
					// console.log('旋转的数据是: --------- ', scope.object.quaternion);
					//console.log('旋转的数据是: --------- ', scope.object.rotation);
					window.curMeshRotation = {
		...
		function onPointerUp( event ) {
			//console.log("源码源码源码源码源码源码onPointerUp");
			event.preventDefault(); // Prevent MouseEvent on mobile

			if ( event.button !== undefined && event.button !== 0 ) return;

			if ( scope._dragging && ( scope.axis !== null ) ) {
